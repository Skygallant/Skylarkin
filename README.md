...
The idea was to have something I could use to base my adventures, from a "Where am I going to charge my car when I get there?" perspective. The app uses the OCM API location data with Chargepoint API price averaging, so inaccuracies are to be expected. OSRM API is used to check whether you'd need a ferry to reach the chargepoints, and to drop them from the search if you do.
